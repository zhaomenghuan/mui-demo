		...
		_tap: function(event) {
			...
			for (; target && target !== document; target = target.parentNode) {
				if (target.tagName === 'A' && target.hash) {	
					var targetHashName = target.hash.replace('#', '');
					var page = document.getElementById(targetHashName);
					if (page && page.classList.contains(CLASS_PAGE)) {
						event.stopPropagation();
						event.detail.gesture.preventDefault();
						if(targetHashName !== this.lastHashName) {
							this.lastHashName = targetHashName; //保存上一次的hash值
							this.go(target.hash);
						};
						break;
					}
